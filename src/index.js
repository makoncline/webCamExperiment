...
function paintCanvas() {
  ...
  return setInterval(() => {
    ...
    const pixles = ctx.getImageData(0, 0, width, height);
    if (prevFrame) {
      // pixles = filter(pixles);
      const newPixles = diff(pixles);
      ctx.putImageData(newPixles, 0, 0);
    }
    prevFrame = pixles;
  }, 16);
}

function filter(pixles) {
  for (let i = 0; i < pixles.data.length; i += 4) {
    pixles.data[i + 0] = pixles.data[i + 0] += 100; //red
    pixles.data[i + 1] = pixles.data[i + 1] += 0; //green
    pixles.data[i + 2] = pixles.data[i + 2] += 0; //blue
    pixles.data[i + 3] = pixles.data[i + 3] += 100; //alpha
  }
  return pixles;
}

function diff(pixles) {
  const diff = ctx.createImageData(canvas.width, canvas.height);
  const changedPixles = pixelmatch(
    prevFrame.data,
    pixles.data,
    diff.data,
    canvas.width,
  ...
  const totalPixles = pixles.data.length / 4;
  const percentDiff = (changedPixles / totalPixles) * 100;
  return diff;
}
